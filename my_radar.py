...
st.set_page_config(page_title="獵人戰情室：終極波段導航儀 5.5", layout="wide")
st.title("🎯 台股波段轉折導航儀")
...
if st.sidebar.button("發送測試警報 🚀"):
    if line_token and line_user_id:
        if send_line_message("✅ 【戰情室廣播】指揮官，雷達 5.5 週末完美版已上線，全指標監控中！", line_token, line_user_id):
...
# --- 抓取所有戰區資料 (🔥分離運算：解決週末 BTC 導致傳統資產歸零問題) ---

# 1. 傳統資產 (週末休市，過濾掉空的週末列)
...
# 2. 比特幣 (24小時全年無休)
btc_data = yf.download("BTC-USD", period="10d")['Close']

# 3. 台股大盤與櫃買 (週末休市)
tw_tickers = ["^TWII", "^TWOII"]
tw_data = yf.download(tw_tickers, period="2mo")['Close'].dropna(how='all').ffill()
...
with c2:
    st.info("大戶避險")
    st.metric("VVIX 指數 (>110警戒)", f"{trad_latest['^VVIX']:.1f}", delta=f"{vvix_delta:.1f}", delta_color="inverse")
    if trad_latest['^VVIX'] > 110: 
...
with c3:
    st.info("黑天鵝")
    st.metric("SKEW 指數 (>140危險)", f"{trad_latest['^SKEW']:.1f}", delta=f"{skew_delta:.1f}", delta_color="inverse")
    if trad_latest['^SKEW'] > 140:
        st.error("💣 核彈預警飆升！")
        st.markdown("**🎯 您的動作**：\n巨鱷正豪賭崩盤！切勿滿倉，鎖死現金避險。")
    else:
        st.success("✅ 尾部風險低。")
...
with u1:
    st.info("半導體羅盤")
    st.metric("費城半導體", f"{trad_latest['^SOX']:.2f}", f"{sox_pct:.2f}%")
with u2:
    st.info("科技股情緒")
    st.metric("那斯達克 100", f"{trad_latest['^NDX']:.2f}", f"{ndx_pct:.2f}%")
with u3:
    st.info("大盤先鋒")
...
hist_data = yf.download(history_tickers, period="6mo")['Close'].dropna(how='all').ffill()
