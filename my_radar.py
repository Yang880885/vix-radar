...
with col1:
    ...
    if ratio_vix_vix3m > 1: st.error("🚨 逆價差爆發！瞄準長下影線準備 30% 抄底！")
    else: st.success("✅ 正價差，天下太平，抱緊多單。")
with col2:
    st.info("戰區 2：大戶避險情緒")
    st.metric(label="VVIX 指數 (>110警戒)", value=f"{latest['^VVIX']:.1f}")
    if latest['^VVIX'] > 110: st.warning("⚠️ 聰明錢偷買保險！拉高停利點。")
    else: st.success("✅ 大戶情緒正常。")
with col3:
    st.info("戰區 3：黑天鵝雷達")
    st.metric(label="SKEW 指數 (>140危險)", value=f"{latest['^SKEW']:.1f}")
with col4:
    st.info("戰區 4：短線均值回歸")
    st.metric(label="9D-VIX 乖離 (>5超殺)", value=f"{(latest['^VIX9D'] - latest['^VIX']):.2f}")
