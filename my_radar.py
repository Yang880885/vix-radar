...
# --- 📊 戰區 5：歷史回測 ---
st.markdown("### 📊 戰區 5：歷史回測 (戰略上帝視角)")
st.caption("💡 戰術邏輯：歷史不會簡單重複，但總是踏著相同的韻腳。用過去的勝率，強化您當下扣下扳機的底氣。")

@st.cache_data(ttl=3600) # 快取歷史資料1小時，確保網頁極速流暢
def get_historical_data():
    hist_tickers = ["^VIX", "^VIX3M", "^TWII", "TWD=X"]
    # 抓取過去一年的歷史資料來對比
    hist_df = yf.download(hist_tickers, period="1y")['Close'].ffill()
    return hist_df

with st.spinner("⏳ 正在調閱歷史戰情檔案..."):
    hist_data = get_historical_data()

# 建立兩個分頁來放置圖表，保持版面乾淨
tab1, tab2 = st.tabs(["🎯 恐慌抄底印證圖 (VIX vs 大盤)", "💸 熱錢抽送水雷達 (匯率 vs 大盤)"])

with tab1:
    st.markdown("#### 核心恐慌比值 VS 台股大盤 (近一年)")
    st.info("💡 觀察重點：當紅色恐慌比值「突破 1.0 虛線」時，對照藍色的大盤，是否都剛好落在波段最低點？")
    hist_ratio = hist_data['^VIX'] / hist_data['^VIX3M']
    
    fig1 = make_subplots(specs=[[{"secondary_y": True}]])
    fig1.add_trace(go.Scatter(x=hist_data.index, y=hist_data['^TWII'], name="台股大盤", line=dict(color='#2962ff', width=2)), secondary_y=False)
    fig1.add_trace(go.Scatter(x=hist_data.index, y=hist_ratio, name="VIX恐慌比值", line=dict(color='rgba(255, 59, 59, 0.8)', width=2)), secondary_y=True)
    
    # 畫出 1.0 的紅色狙擊警戒線
    fig1.add_hline(y=1.0, line_dash="dash", line_color="red", secondary_y=True, annotation_text="狙擊扳機觸發線 (1.0)")
    
    fig1.update_layout(height=450, hovermode="x unified", margin=dict(l=0, r=0, t=30, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    st.plotly_chart(fig1, use_container_width=True)

with tab2:
    st.markdown("#### 美元/台幣匯率 VS 台股大盤 (近一年)")
    st.info("💡 觀察重點：這是一場蹺蹺板遊戲。當匯率紅線往上(台幣貶值/外資提款)，大盤藍線通常往下摔；反之亦然。")
    
    fig2 = make_subplots(specs=[[{"secondary_y": True}]])
    fig2.add_trace(go.Scatter(x=hist_data.index, y=hist_data['^TWII'], name="台股大盤", line=dict(color='#2962ff', width=2)), secondary_y=False)
    fig2.add_trace(go.Scatter(x=hist_data.index, y=hist_data['TWD=X'], name="美元/台幣匯率", line=dict(color='#ffb703', width=2)), secondary_y=True)
    
    fig2.update_layout(height=450, hovermode="x unified", margin=dict(l=0, r=0, t=30, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
