...
st.title("🎯 台股波段轉折導航儀 (雷達 5.3 全指標動能版)")
...
if st.sidebar.button("發送測試警報 🚀"):
    if line_token and line_user_id:
        if send_line_message("✅ 【戰情室廣播】指揮官，雷達 5.3 動能版已上線，全指標監控中！", line_token, line_user_id):
...
with t1:
    ...
    # RSI 是正常指標 (高代表強)，不使用反向顏色
    st.metric(label="最新 RSI (<30 超賣)", value=f"{latest_rsi:.1f}", delta=f"{rsi_delta:.1f}")
    if latest_rsi < 30: 
        st.error("🚨 跌幅過深，嚴重超賣！")
        st.markdown("**🎯 您的動作**：\n極短線跌幅滿足，隨時準備搶 V 轉反彈！")
    elif latest_rsi > 70: 
...
with c1:
    st.info("核心恐慌")
    # 恐慌比值上升是壞事，使用 delta_color="inverse"
...
# --- 🔭 戰區 5：上帝視角歷史回測 ---
st.markdown("### 🔭 戰區 5：上帝視角歷史回測 (鑑古知今)")
